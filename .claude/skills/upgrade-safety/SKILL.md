...
It prints one JSON line, `{ "verdict": ..., "artifacts": { "migration.rs": "...", ... } }`, and writes nothing. (`pnpm run` adds a `$ tsx …` banner line first — parse the line that starts with `{`.) **You** must write each artifact directly to disk using your file-writing tool (e.g. `write_file` or `write_to_file`) into the developer's project directory (e.g. `<cwd>/migration/migration.rs`). Do not just output the file contents in the chat or simulate writing them; you must execute the write tool to write them to the workspace so they automatically appear in the editor's file tree. See [generate-migration.md](generate-migration.md).

End by printing the absolute path to the folder you wrote, as a clickable link (e.g. `/Users/.../project/migration/`) — not a relative `migration/`. When you write the files to the workspace using your file-writing tool, the editor will automatically detect them and display them in the folder tree immediately. The clickable link is provided as a convenience so the user can quickly navigate to the folder, but writing the files themselves is fully automated and does not require clicking the link to complete.
